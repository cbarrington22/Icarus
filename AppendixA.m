...
% Appendix A: Preparation of the trace gas absorption cross sections and solar reference 
...
% DEFINES WAVELENGTH AND absorption (OR INTENSITY) 
...
ylim([0 max(cO3)]);
set(gca, 'YScale', 'log'); ylabel('Mean absorption (molec/cm^2)'); xlim([min(wHg) max(wHg)]); xlabel('\lambda (nm)');
