...
inDirFS = '/Users/charlotteb/Documents/Chapter 2/Cross sections/Fraunhofer (for Introduction)/';
...
dp = plot(lambda, SO2, '-'); dp.LineWidth = 1.5; dp.Color = colSO2;
...
fnSolar30 = 'Reference_30_0_FLMS195681__0__11-26-46-904.txt';
fnSolar90 = 'Reference_90_0_FLMS195681__0__10-13-49-165.txt';
% DEFINES WAVELENGTH AND ABSOPRTION (OR INTENSITY) 
% Defines file directory, converts to character array and loads wavelength and absorption OR intensity information into the workspace 
dSolar30 = fullfile(inDirFS, fnSolar30); inSolar30 = char(dSolar30); 
dSolar90 = fullfile(inDirFS, fnSolar90); inSolar90 = char(dSolar90);  
dHg = fullfile(inDirHg, fnHg); inHg = char(dHg); % Hg-spectrum 
if hl >= 1
   [wSolar30, iSolar30] = textread(inSolar30,'%f %f', 'headerlines', hl); % Skips header lines if exist 
   [wSolar90, iSolar90] = textread(inSolar90,'%f %f', 'headerlines', hl); % Skips header lines if exist 
else
end  
subplot(2,1,2)
dp = plot(wSolar30, iSolar30, ':k'); hold on; dp.LineWidth = 1;
dp = plot(wSolar90, iSolar90, '-k'); hold on; dp.LineWidth = 1;
ylim([0 6.2e04]); 
...
Fig = gca; Fig.FontSize = 13; set(gcf,'color','w');
% Save convoluted cross sections to be used later 
fname = fullfile(outDir, 'Introduction_Figures');
save(fname); % Saves all variables  
% Changes name
I0 = csolar; 
Ring = cRing; 
fname = fullfile(outDir, 'Fig1_Fig3_conv_I0_SO2_O3_Ring');
save(fname, 'fnHg', 'fnSolar', 'I0', 'fnSO2', 'SO2', 'fnO3', 'O3', 'fnRing' , 'Ring', 'lambda', 'kx'); 
%% Make any adjustments 
% % SAVES FIGURE
% fnOut = 'Fig3_FraunhoferLines';
% fname = fullfile(outDirFig, fnOut);
% saveas(gca, fname)
