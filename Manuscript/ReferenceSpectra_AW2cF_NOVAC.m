...
% Model version 
% (1) AW2 cropped spatial frequencies 
% - Real 
...
outDir = '/Users/charlotteb/Documents/Chapter 2/NOVAC spectra/AW2/';
outDirFig = '/Users/charlotteb/Documents/Chapter 2/NOVAC spectra/AW2/Figures/';
...
% SAVES variables 
fname = fullfile(outDir, 'ReferenceSpectra_AW2cF_G_NOVAC');
...
for s = 1:length(spectraList)
    if s == 2 % don't analyse dark scan with is "00001_0.STD", second in list 
        ...
        if s == 28 % Index for target d (2080 ppmm at zenith) - same used for figures - now index for zenith scan "00027_0.STD"
           ...
           box on
           % SAVES FIGURE
           fnOut = 'ReferenceSpectra_AW2cF_dExample_NOVAC_zenith';
        ...
        if s == 14 || s == 28 || s == 35 || s == 49 % Plots residuals for -39, zenith, 39 and 79 degrees 
           ...
           box on
           % SAVES FIGURE
           fnOut = 'ReferenceSpectra_AW2cF_Residual_NOVAC';
           fname = fullfile(outDirFig, fnOut);
           saveas(gca, fname)
        else
        end
...
% SAVES variables 
fname = fullfile(outDir, 'ReferenceSpectra_AW2cF_mEst_NOVAC_Masaya_1417');
...
% SAVES variables 
fname = fullfile(outDir, 'ReferenceSpectra_AW2cF_mEst_NOVAC_Masaya_1941');
save(fname, 'G_real', 'G_realimag', 'elevAn', 'lambda', 'F', 'Results_mEst_real', 'Results_res_real', 'Results_stdErr_real', 'wavL', 'wavH', 'Ft', 'Fb'); 
toc 
