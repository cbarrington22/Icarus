...
xlim([min(wHg) max(wHg)]); xlabel('\lambda (nm)'); ylabel('Cycles/\lambda'); set(gca,'YScale','log');
Fig = gca; Fig.FontSize = 15; set(gcf,'color','w');
title('CWT of SO_2 absorption cross section')
subplot(1,3,2)
[wtO3, ~ , ~] = cwt(O3, Fs); % O3
wtO3(idxCoi) = NaN; 
...
xlim([min(wHg) max(wHg)]); xlabel('\lambda (nm)'); ylabel('Cycles/\lambda'); set(gca,'YScale','log');
Fig = gca; Fig.FontSize = 15; set(gcf,'color','w');
title('CWT of O_3 absorption cross section')
subplot(1,3,3) 
...
WuAw = wHg(idxWuAw); 
% FREQUENCY
% Frequency levels in F are decending, so low row indicies represent higher frequencies and higher row indicies represent low frequencies 
...
fprintf('Prepared CWT of absoprtion cross sections and solar reference saved in %s\n', outDir); % Displays message to user 
